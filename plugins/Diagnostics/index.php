 ...
 * @description This file takes a look at your PhotosManager-configuration and displays all errors it can find.
 */
...
require($photosManager . 'php/helpers/hasPermissions.php');
...
// Load config
if (!file_exists(PHOTOS_MANAGER_CONFIG_FILE)) exit('Error: Configuration not found. Please install PhotosManager for additional tests');
...
// Check mysql version
if ($database->server_version<50500) echo('Warning: PhotosManager uses the GBK charset to avoid sql injections on your MySQL version. Please update to MySQL 5.5 or higher to enable UTF-8 support.' . PHP_EOL);
...
// Output
if ($error==='') echo('No critical problems found. PhotosManager should work without problems!' . PHP_EOL);
else             echo $error;
...
if ((isset($_SESSION['login'])&&$_SESSION['login']===true)&&
	...
	// Load json
	$json = file_get_contents(PHOTOS_MANAGER_SRC . 'package.json');
	...
	echo('PhotosManager Version:  ' . $json['version'] . PHP_EOL);

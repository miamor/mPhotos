...
final class Database {
	 ...
	 */
	private function __construct($host, $user, $password, $name = 'photos_manager', $dbTablePrefix) {
	 ...
	 * @return boolean Returns true when successful.
	 */
	public static function createDatabase($connection, $name = 'photos_manager') {
	...
	private static function createTables($connection) {
		...
		$query  = self::prepare($connection, 'SELECT * FROM ?, ?, ?, ? LIMIT 0', array(PHOTOS_MANAGER_TABLE_PHOTOS, PHOTOS_MANAGER_TABLE_ALBUMS, PHOTOS_MANAGER_TABLE_SETTINGS, PHOTOS_MANAGER_TABLE_LOG));
		$result = self::execute($connection, $query, null, null);
		if ($result!==false) return true;

		// Check if log table exists
		$exist  = self::prepare($connection, 'SELECT * FROM ? LIMIT 0', array(PHOTOS_MANAGER_TABLE_LOG));
		...
		if ($result===false) {
			...
			// Create table
			$query  = self::prepare($connection, $query, array(PHOTOS_MANAGER_TABLE_LOG));
			$result = self::execute($connection, $query, null, null);
		...
		$exist  = self::prepare($connection, 'SELECT * FROM ? LIMIT 0', array(PHOTOS_MANAGER_TABLE_SETTINGS));
		...
		if ($result===false) {
			...
			if ($query===false) {
				Log::error($connection, __METHOD__, __LINE__, 'Could not load query for table settings');
				return false;
			}

			// Create table
			$query  = self::prepare($connection, $query, array(PHOTOS_MANAGER_TABLE_SETTINGS));
			...
			if ($query===false) {
				Log::error($connection, __METHOD__, __LINE__, 'Could not load content-query for table settings');
				return false;
			}
			...
			$query      = self::prepare($connection, "UPDATE `?` SET `value` = '?' WHERE `key` = 'identifier' LIMIT 1", array(PHOTOS_MANAGER_TABLE_SETTINGS, $identifier));
			$result     = self::execute($connection, $query, __METHOD__, __LINE__);
		...
		$exist  = self::prepare($connection, 'SELECT * FROM ? LIMIT 0', array(PHOTOS_MANAGER_TABLE_ALBUMS));
		$result = self::execute($connection, $exist, __METHOD__, __LINE__);
		...
		if ($result===false) {
			...
			if ($query===false) {
				Log::error($connection, __METHOD__, __LINE__, 'Could not load query for table albums');
				return false;
			}

			// Create table
			$query  = self::prepare($connection, $query, array(PHOTOS_MANAGER_TABLE_ALBUMS));
			$result = self::execute($connection, $query, __METHOD__, __LINE__);
		...
		$exist  = self::prepare($connection, 'SELECT * FROM ? LIMIT 0', array(PHOTOS_MANAGER_TABLE_PHOTOS));
		$result = self::execute($connection, $exist, __METHOD__, __LINE__);
		...
		if ($result===false) {
			...
			if ($query===false) {
				Log::error($connection, __METHOD__, __LINE__, 'Could not load query for table photos');
				return false;
			}

			// Create table
			$query  = self::prepare($connection, $query, array(PHOTOS_MANAGER_TABLE_PHOTOS));
			$result = self::execute($connection, $query, __METHOD__, __LINE__);
	...
	private static function update($connection, $dbName) {
		...
		// Get current version
		$query  = self::prepare($connection, "SELECT * FROM ? WHERE `key` = 'version'", array(PHOTOS_MANAGER_TABLE_SETTINGS));
		$result = self::execute($connection, $query, __METHOD__, __LINE__);
	...
	public static function setVersion($connection, $version) {
		...
		$query  = self::prepare($connection, "UPDATE ? SET value = '?' WHERE `key` = 'version'", array(PHOTOS_MANAGER_TABLE_SETTINGS, $version));
		$result = self::execute($connection, $query, __METHOD__, __LINE__);

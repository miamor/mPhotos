...
final class Photo
{
    ...
    public function add(array $files, $albumID = 0, $returnOnError = false)
    {
        ...
        if ($info['title'] === '') {
            $info['title'] = substr(basename($file['name'], $extension), 0, 100);
        }
        ...
        if ($exists === false) {
            ...
            if ($file['type'] === 'image/jpeg' && isset($info['orientation']) && $info['orientation'] !== '') {
                ...
                if ($adjustFile !== false) {
                    ...
                    Log::notice(Database::get(), __METHOD__, __LINE__, 'Skipped adjustment of photo (' . $info['title'] . ')');
                }
            ...
            // Create Medium
            if ($this->createMedium($path, $photo_name, $info['width'], $info['height'])) {
                $medium = 1;
            } else {
                $medium = 0;
            }
    ...
    private function exists($checksum, $photoID = null)
    {
        ...
        if (isset($photoID)) {
            $query = Database::prepare(Database::get(), "SELECT id, url, thumbUrl, medium, album FROM ? WHERE checksum = '?' AND id <> '?' LIMIT 1", array(PHOTOS_MANAGER_TABLE_PHOTOS, $checksum, $photoID));
        } else {
            $query = Database::prepare(Database::get(), "SELECT id, url, thumbUrl, medium, album FROM ? WHERE checksum = '?' LIMIT 1", array(PHOTOS_MANAGER_TABLE_PHOTOS, $checksum));
        }
    ...
    private function createThumb($url, $filename, $type, $width, $height)
    {
        ...
        if (Settings::hasImagick()) {
            ...
            // Create new image
            switch ($type) {
                case 'image/jpeg':$sourceImg = imagecreatefromjpeg($url);
                    break;
                case 'image/png':$sourceImg = imagecreatefrompng($url);
                    break;
                case 'image/gif':$sourceImg = imagecreatefromgif($url);
                    break;
    ...
    private function createMedium($url, $filename, $width, $height)
    {
        ...
        if (($error === false) &&
            ...
            // Save image
            try { $medium->writeImage($newUrl);} catch (ImagickException $err) {
    ...
    public static function prepareData(array $data)
    {
        ...
        // Parse medium
        if ($data['medium'] === '1') {
            $photo['medium'] = $PHOTOS_MANAGER_URL_UPLOADS_MEDIUM__ . $data['url'];
        } else {
            $photo['medium'] = '';
        }
    ...
    public function get($albumID)
    {
        ...
        // Get photo
        $query = Database::prepare(Database::get(), "SELECT * FROM ? WHERE id = '?' LIMIT 1", array(PHOTOS_MANAGER_TABLE_PHOTOS, $this->photoIDs));
        ...
        // Parse medium
        if ($photo['medium'] === '1') {
            $photo['medium'] = $this->PHOTOS_MANAGER_URL_UPLOADS_MEDIUM . $photo['url'];
        } else {
            $photo['medium'] = '';
        }
        ...
        if ($albumID != 'false') {
            ...
            if ($photo['album'] !== '0') {
                ...
                // Get album
                $query = Database::prepare(Database::get(), "SELECT public FROM ? WHERE id = '?' LIMIT 1", array(PHOTOS_MANAGER_TABLE_ALBUMS, $photo['album']));
    ...
    public function getInfo($url)
    {
        ...
        if (isset($iptcArray['APP13'])) {
            ...
            if (is_array($iptcInfo)) {
                ...
                } else if (!empty($iptcInfo['2#005'][0])) {
        ...
        if ($info['mime'] == 'image/jpeg') {
            $exif = @exif_read_data($url, 'EXIF, IFD0', false, false);
        } else {
            $exif = false;
        }
    ...
    public function setTitle($title = 'Untitled')
    {
        ...
        // Set title
        $query = Database::prepare(Database::get(), "UPDATE ? SET title = '?' WHERE id IN (?)", array(PHOTOS_MANAGER_TABLE_PHOTOS, $title, $this->photoIDs));
        $result = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
    ...
    public function setDescription($description)
    {
        ...
        // Set description
        $query = Database::prepare(Database::get(), "UPDATE ? SET description = '?' WHERE id IN ('?')", array(PHOTOS_MANAGER_TABLE_PHOTOS, $description, $this->photoIDs));
        $result = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
    ...
    public function setStar()
    {
        ...
        // Get photos
        $query = Database::prepare(Database::get(), "SELECT id, star FROM ? WHERE id IN (?)", array(PHOTOS_MANAGER_TABLE_PHOTOS, $this->photoIDs));
        $photos = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
        ...
        while ($photo = $photos->fetch_object()) {
            ...
            // Set star
            $query = Database::prepare(Database::get(), "UPDATE ? SET star = '?' WHERE id = '?'", array(PHOTOS_MANAGER_TABLE_PHOTOS, $star, $photo->id));
            $result = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
    ...
    public function getPublic($password)
    {
        ...
        // Get photo
        $query = Database::prepare(Database::get(), "SELECT public, album FROM ? WHERE id = '?' LIMIT 1", array(PHOTOS_MANAGER_TABLE_PHOTOS, $this->photoIDs));
        $photos = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
    ...
    public function setPublic()
    {
        ...
        // Get public
        $query = Database::prepare(Database::get(), "SELECT public FROM ? WHERE id = '?' LIMIT 1", array(PHOTOS_MANAGER_TABLE_PHOTOS, $this->photoIDs));
        $photos = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
        ...
        // Set public
        $query = Database::prepare(Database::get(), "UPDATE ? SET public = '?' WHERE id = '?'", array(PHOTOS_MANAGER_TABLE_PHOTOS, $public, $this->photoIDs));
        $result = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
    ...
    public function setAlbum($albumID)
    {
        ...
        // Set album
        $query = Database::prepare(Database::get(), "UPDATE ? SET album = '?' WHERE id IN (?)", array(PHOTOS_MANAGER_TABLE_PHOTOS, $albumID, $this->photoIDs));
        $result = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
    ...
    public function setTags($tags)
    {
        ...
        // Set tags
        $query = Database::prepare(Database::get(), "UPDATE ? SET tags = '?' WHERE id IN (?)", array(PHOTOS_MANAGER_TABLE_PHOTOS, $tags, $this->photoIDs));
        $result = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
    ...
    public function duplicate()
    {
        ...
        // Get photos
        $query = Database::prepare(Database::get(), "SELECT id, checksum FROM ? WHERE id IN (?)", array(PHOTOS_MANAGER_TABLE_PHOTOS, $this->photoIDs));
        $photos = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
    ...
    public function delete()
    {
        ...
        while ($photo = $photos->fetch_object()) {
            ...
            // Delete db entry (photo)
            $query = Database::prepare(Database::get(), "DELETE FROM ? WHERE id = '?'", array(PHOTOS_MANAGER_TABLE_PHOTOS, $photo->id));
            $result = Database::execute(Database::get(), $query, __METHOD__, __LINE__);

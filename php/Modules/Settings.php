...
final class Settings
{
    ...
    /**
     * @return array Returns the settings of PhotosManager.
     */
    public static function get()
    {
        ...
        // Execute query
        $query = Database::prepare(Database::get(), "SELECT * FROM ?", array(PHOTOS_MANAGER_TABLE_SETTINGS));
    ...
    private static function set($key, $value, $row = false)
    {
        ...
        if ($row === false) {
            ...
            $query = Database::prepare(Database::get(), "UPDATE ? SET value = '?' WHERE `key` = '?'", array(PHOTOS_MANAGER_TABLE_SETTINGS, $value, $key));

        } elseif ($row === true) {

            // Do not prepare $value because it has already been escaped or is a true statement
            $query = Database::prepare(Database::get(), "UPDATE ? SET value = '$value' WHERE `key` = '?'", array(PHOTOS_MANAGER_TABLE_SETTINGS, $key));
    ...
    public static function setLogin($oldPassword = '', $username, $password)
    {
        ...
        if ($oldPassword === self::get()['password'] || self::get()['password'] === crypt($oldPassword, self::get()['password'])) {
            ...
            // Save username
            if (self::setUsername($username) === false) {
                Response::error('Updating username failed!');
            }

            // Save password
            if (self::setPassword($password) === false) {
                Response::error('Updating password failed!');
            }

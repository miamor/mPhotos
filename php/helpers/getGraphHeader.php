...
function getGraphHeader($photoID) {
	...
	$query  = Database::prepare(Database::get(), "SELECT title, description, url, medium FROM ? WHERE id = '?'", array(PHOTOS_MANAGER_TABLE_PHOTOS, $photoID));
	...
	$return .= '<meta name="description" content="' . $row->description . ' - via PhotosManager">';
	...
	$return .= '<meta property="og:description" content="' . $row->description . ' - via PhotosManager">';

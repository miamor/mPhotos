...
$query  = Database::prepare($connection, "UPDATE `?` SET `value` = '' WHERE `key` = 'username' LIMIT 1", array(PHOTOS_MANAGER_TABLE_SETTINGS));
...
$query  = Database::prepare($connection, "UPDATE `?` SET `value` = '' WHERE `key` = 'password' LIMIT 1", array(PHOTOS_MANAGER_TABLE_SETTINGS));
$result = Database::execute($connection, $query, 'update_030000', __LINE__);
...
$query  = Database::prepare($connection, "UPDATE `?` SET `public` = 0, `password` = NULL", array(PHOTOS_MANAGER_TABLE_ALBUMS));
$result = Database::execute($connection, $query, 'update_030000', __LINE__);

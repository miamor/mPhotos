...
$query  = Database::prepare($connection, "ALTER TABLE `?` CHANGE `title` `title` VARCHAR( 100 ) NOT NULL DEFAULT ''", array(PHOTOS_MANAGER_TABLE_PHOTOS));
...
$query  = Database::prepare($connection, "ALTER TABLE `?` CHANGE `title` `title` VARCHAR( 100 ) NOT NULL DEFAULT ''", array(PHOTOS_MANAGER_TABLE_ALBUMS));
$result = Database::execute($connection, $query, 'update_030001', __LINE__);

if ($result===false) Response::error('Could not change length of album title in database!');

// Add album sorting to settings
$query  = Database::prepare($connection, "SELECT `key` FROM `?` WHERE `key` = 'sortingAlbums' LIMIT 1", array(PHOTOS_MANAGER_TABLE_SETTINGS));
$result = Database::execute($connection, $query, 'update_030001', __LINE__);
...
if ($result->num_rows===0) {
	...
	$query  = Database::prepare($connection, "INSERT INTO `?` (`key`, `value`) VALUES ('sortingAlbums', 'ORDER BY id DESC')", array(PHOTOS_MANAGER_TABLE_SETTINGS));
	$result = Database::execute($connection, $query, 'update_030001', __LINE__);
...
$query  = Database::prepare($connection, "UPDATE ? SET `key` = 'sortingPhotos' WHERE `key` = 'sorting' LIMIT 1", array(PHOTOS_MANAGER_TABLE_SETTINGS));
$result = Database::execute($connection, $query, 'update_030001', __LINE__);

if ($result===false) Response::error('Could not rename photo sorting row in database!');

// Add identifier to settings
$query  = Database::prepare($connection, "SELECT `key` FROM `?` WHERE `key` = 'identifier' LIMIT 1", array(PHOTOS_MANAGER_TABLE_SETTINGS));
$result = Database::execute($connection, $query, 'update_030001', __LINE__);
...
if ($result->num_rows===0) {
	...
	$query      = Database::prepare($connection, "INSERT INTO `?` (`key`, `value`) VALUES ('identifier', '?')", array(PHOTOS_MANAGER_TABLE_SETTINGS, $identifier));
	$result     = Database::execute($connection, $query, 'update_030001', __LINE__);

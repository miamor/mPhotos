...
$query  = Database::prepare($connection, "SELECT `key` FROM `?` WHERE `key` = 'skipDuplicates' LIMIT 1", array(PHOTOS_MANAGER_TABLE_SETTINGS));
...
if ($result->num_rows===0) {
	...
	$query  = Database::prepare($connection, "INSERT INTO `?` (`key`, `value`) VALUES ('skipDuplicates', '0')", array(PHOTOS_MANAGER_TABLE_SETTINGS));
	$result = Database::execute($connection, $query, 'update_030003', __LINE__);
